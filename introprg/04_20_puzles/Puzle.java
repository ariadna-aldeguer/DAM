...
/*
 * Clase que té tots els métodes a fer amb puzle 
 * crear, duplicar, borrar, etc. 
 */
...
public class Puzle{
    ...
    public static Escacs llegeixPuzle(String nomPuzle) throws IOException {
        ...
        char[][] copia = new char [linia.length()][linia.length()];
        for(int i = linia.length()-1; i >= 0; i--){
            if(linia == null) break;
            for(int j = 0; j < linia.length(); j++){
                    copia[i][j] = linia.charAt(j);
                }
            linia = entrada.readLine();
        }
        entrada.close();  
        Escacs escacs = new Escacs(copia);
    ...
    public static void processaAdd(String[] args) throws IOException{
        ...
        Escacs escacs = llegeixPuzle(args[1]);
    ...
    public static void processaMv(String[] args) throws IOException{
        ...
        if (escacs.mouFigura(filaOrigen, columnaOrigen, filaDesti, columnaDesti)){
            escriuPuzle(args[1], escacs);
            return;
        } else {
    ...
    //del[0] «nom»[1] «fila»[2] «columna»[3]
    
        if (args.length != 4){
            mostrarErrorComandaDel();
            return;
        }   
        if (!existeixPuzle(args[1])) {
            mostraErrorPuzleDesconegut();
        ...
        if (escacs.eliminaFigura(fila, columna)){
            ...
            mostraErrorNoElimina();
            return;
        }
    }    
    public static void mostraErrorNoElimina(){
    ...
    public static void mostrarErrorComandaDel(){
